...
- **Promo banner** — a black, rounded, responsive frame at the top of the documentary. It shows `assets/videos/promo.mp4` (autoplay, muted, looped, no controls) if that file exists, otherwise falls back to `assets/images/sections/promo.jpg`, otherwise stays an empty black banner. Swap either file any time — no code changes needed.
