...
(() => {
  ...
  function protectMediaElement(el) {
    ...
    if (el.tagName === 'VIDEO') {
      ...
      const exitIfThisVideo = () => {
        if (document.fullscreenElement === el && document.exitFullscreen) {
          document.exitFullscreen().catch(() => {});
        } else if (document.webkitFullscreenElement === el && el.webkitExitFullscreen) {
          el.webkitExitFullscreen();
        }
      };
      el.addEventListener('fullscreenchange', exitIfThisVideo);
      el.addEventListener('webkitfullscreenchange', exitIfThisVideo);
      // Some mobile browsers auto-fullscreen <video> on play; back out.
      el.addEventListener('webkitbeginfullscreen', () => {
        if (el.webkitExitFullscreen) el.webkitExitFullscreen();
      });
    }
  }
  ...
  /* ---------------------------------------------------
     9. MEDIA PROTECTION
        Best-effort deterrents against casual saving/
        copying of images and video. This can never be
        airtight client-side (anything rendered in a
        browser can technically be captured), but it
        blocks the easy, accidental paths: right-click
        save, drag-and-drop, the iOS/Android long-press
        callout, native video download/PiP/remote-
        playback/fullscreen controls, and double-click-
        to-fullscreen.
     --------------------------------------------------- */
  document.querySelectorAll('img, video').forEach(protectMediaElement);

  // Document-level fallback: catches the context menu / drag start
  // on any media added or swapped in later (e.g. the promo banner's
  // image/video src is set dynamically above).
